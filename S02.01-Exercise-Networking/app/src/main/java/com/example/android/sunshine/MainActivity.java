...
*/
    ...
    private void loadWeatherData() {
        String locate = SunshinePreferences.getPreferredWeatherLocation(this);
        new NetworkBuilder().execute(locate);
    }
    // COMPLETED (5) Create a class that extends AsyncTask to perform network requests
    public class NetworkBuilder extends AsyncTask<String, Void, String[]>{
        @Override
        protected String[] doInBackground(String... params) {

            String url_locater = params[0];
            URL wurl = NetworkUtils.buildUrl(url_locater);

            try {
               String jReply = NetworkUtils.getResponseFromHttpUrl(wurl);

                String[]  wData = OpenWeatherJsonUtils.getSimpleWeatherStringsFromJson(MainActivity.this,jReply);
                return  wData;
    ...
    // CompletedTODO (6) Override the doInBackground method to perform your network requests
    // CompletedTODO (7) Override the onPostExecute method to display the results of the network request
}